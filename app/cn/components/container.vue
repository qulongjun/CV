<template>
    <div id="pagepiling">
        <div class="section center" id="home">
            <img id="avatar" src="http://cdn.qulongjun.cn/touxiang.jpg"/>
            <h1 id="title">瞿龙俊</h1>
            <p id="subtitle">一腔热血的激情 · 一夜失眠的创意 · 一个像素的执着</p>
            <h4>前端研发工程师@触宝科技</h4>
            <h6><a href="http://blog.qulongjun.cn" target="_blank">博客</a></h6>
            <p><a href="mailto:Longjun.qu@cootek.cn">Longjun.qu@cootek.cn</a></p>
        </div>

        <div class="section" id="project">
            <h1>项目经验</h1>
            <div class="row">
                <div class="col-2 center">
                    <a class="circle-img" href="https://github.com/qulongjun/Blog" target="_blank">
                        <img src="../img/blog.png" alt="Blog"/>
                    </a>
                </div>
                <div class="col-4">
                    <h2><a href="https://github.com/qulongjun/Blog" target="_blank">Enar</a></h2>
                    <p>一款轻博客，记录工作中的琐碎</p>
                </div>
                <div class="col-2 center">
                    <a class="circle-img" href="https://github.com/qulongjun/DataEngine_SDK" target="_blank">
                        <img src="../img/connect.png" alt="DataEngine_SDK"/>
                    </a>
                </div>
                <div class="col-4">
                    <h2><a href="https://github.com/qulongjun/DataEngine_SDK" target="_blank">DataEngine SDK</a></h2>
                    <p>基于Javascript的可视化数据库前后端通信SDK。</p>
                </div>
                <div class="clearboth"></div>
            </div>
            <div class="row">
                <div class="col-2 center">
                    <a class="circle-img" href="https://github.com/qulongjun/JTable" target="_blank">
                        <img src="../img/table.png" alt="JTable"/>
                    </a>
                </div>
                <div class="col-4">
                    <h2><a href="https://github.com/qulongjun/JTable" target="_blank">JTable</a></h2>
                    <p>基于JQuery的轻量级动态表格组件。</p>
                </div>
                <div class="col-2 center">
                    <a class="circle-img" href="https://github.com/qulongjun/Jun-CLI"
                       target="_blank">
                        <img src="../img/build.png" alt="Jun-CLI"/>
                    </a>
                </div>
                <div class="col-4">
                    <h2><a href="https://github.com/qulongjun/Jun-CLI" target="_blank">Jun CLI</a></h2>
                    <p>更易上手的前端构建工具，基于Vue和Webpack封装</p>
                </div>
                <div class="clearboth"></div>
            </div>
            <div class="row">
                <div class="col-2 center">
                    <a class="circle-img" href="https://github.com/LIMSBuilder/LIMS_Front" target="_blank">
                        <img src="../img/laboratory.png" alt="LIMS"/>
                    </a>
                </div>
                <div class="col-4">
                    <h2><a href="https://github.com/LIMSBuilder/LIMS_Front" target="_blank">LIMS</a></h2>
                    <p>外包项目，实验室信息管理系统</p>
                </div>
                <div class="col-2 center">
                    <a class="circle-img" href="https://github.com/qulongjun/JQuery_Pagination" target="_blank">
                        <img src="../img/page.png" alt="JQuery_Pagination"/>
                    </a>
                </div>
                <div class="col-4">
                    <h2><a href="https://github.com/qulongjun/JQuery_Pagination" target="_blank">Pagination</a></h2>
                    <p>一款简易的页码生成控件，支持前后端分离</p>
                </div>
                <div class="clearboth"></div>
            </div>
            <p class="center">更多精彩，敬请移步 <a href="http://github.com/qulongjun" target="_blank"
                                           class="highlight">GitHub</a>
            </p>
        </div>

        <div class="section center" id="profession">
            <h1>擅长技术</h1>
            <div class="tag-panel">
                <div class="tag expert">CSS</div>
                <div class="tag familiar">SCSS</div>
            </div>
            <div class="tag-panel">
                <div class="tag expert">JavaScript</div>
                <div class="tag expert">jQuery</div>
                <div class="tag expert">Bootstrap</div>
                <div class="tag familiar">WebPack</div>
                <div class="tag familiar">Vue</div>
            </div>
            <div class="tag-panel">
                <div class="tag familiar">HTML5</div>
                <div class="tag familiar">Mock</div>
                <div class="tag">Nightmare</div>
            </div>
            <div class="tag-panel">
                <div class="tag expert">网页设计</div>
                <div class="tag familiar">前后端分离</div>
                <div class="tag familiar">组件开发</div>
            </div>
            <div class="tag-panel">
                <div class="tag expert">Java</div>
                <div class="tag expert">JFinal</div>
            </div>
        </div>

        <div class="section center">
            <h1>教育背景</h1>
            <h4>江苏大学 硕士</h4>
            <p>计算机科学与技术专业</p>
            <h3>数据挖掘实验室</h3>
            <p>2017年6月毕业</p>
            <p>研究课题数据存储、数据挖掘相关</p>
        </div>

        <div class="section center">
            <h1>实习经历</h1>

            <h4>前端工程师@触宝科技</h4>
            <p>2017年6月 - 至今</p>
            <p>Feeds流和数据交互</p>

            <h4>前端工程师@Epub360</h4>
            <p>2015年12月 - 2016年7月</p>
            <p>重构和组件开发</p>

            <h4>软件开发工程师@邦达诚科技</h4>
            <p>2015年8月 - 2015年12月</p>
            <p>研发、测试、运营</p>
            <p>负责长期运维</p>

        </div>

        <div class="section center">
            <h1>目前供职</h1>
            <h2>前端研发工程师@触宝科技</h2>
            <h4>从事 <a href="http://www.chubao.cn/dialer/index.html">Feeds流</a> 相关工作</h4>
            <p>工作地点：上海</p>
            <p>涉及大数据、视频、任务系统、JavaScript、Node.js 等</p>
        </div>

        <div class="section center">
            <h1>了解更多</h1>
            <h2><a href="http://blog.qulongjun.cn" target="_blank">博客</a> | <a
                    href="http://www.linkedin.com/in/qulongjun" target="_blank">LinkedIn</a></h2>
            <p>邮箱: <a href="mailto:Longjun.qu@cootek.cn" target="_blank">Longjun.qu@cootek.cn</a></p>

            <h2><a href="http://cdn.qulongjun.cn/resume_V1.0.pdf" target="_blank" class="highlight">下载简历</a></h2>
            <div id="contact-panel">
                <a class="social-share-element social-github" href="https://github.com/qulongjun" target="_blank"
                   title="Follow me on Github"></a>
                <a class="social-share-element social-wechat"
                   href="weixin://contacts/profile/qulongjun1992" target="_blank"
                   title="Follow me on Wechat"></a>
                <a class="social-share-element social-zhihu" href="https://www.zhihu.com/people/qu-long-jun/activities"
                   target="_blank"
                   title="Follow me on Zhihu"></a>

            </div>
        </div>
    </div>
</template>
<script type="es6">
    import {mobileCheck} from '../js/index';
    module.exports = {
        mounted: () => {
            if (mobileCheck()) {
                $('html').addClass('mobile');
            } else {
                var cn = ['首页', '项目', '技能', '教育', '实习', '供职', '联系'];
                $('.section').addClass('initing');
                $('#pagepiling').pagepiling({
                    loopBottom: true,
                    navigation: {
                        'position': 'left',
                        'tooltips': cn
                    },
                    onLeave: function (index, nextIndex, direction) {
                        var $pages = $('.section');
                        for (var i = 0; i < $pages.length; ++i) {
                            if (i + 1 !== nextIndex) {
                                $($pages[i]).addClass('leaving');
                            } else {
                                $($pages[i]).removeClass('leaving');
                            }
                        }
                    }
                });
                $('.initing').removeClass('initing');
            }
        }
    };
</script>